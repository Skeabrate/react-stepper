  ...
  return (
    <Wrapper>
      <StyledContent>
        ...
        <p>{finish ? `All steps competed - you're finished` : `Step ${step}`}</p>
        ...
        <StyledButtons finish={finish}>
          ...
          {step !== stepsData.length && (
            <div>
              {step !== 1 && <button onClick={skipHandler}>SKIP</button>}
              <StyledHighlightButton onClick={nextHandler}>NEXT</StyledHighlightButton>
            </div>
          )}

          {step === stepsData.length && !finish && (
            <StyledHighlightButton onClick={finishHandler}>FINISH</StyledHighlightButton>
          )}

          {finish && <StyledHighlightButton onClick={resetHandler}>RESET</StyledHighlightButton>}
        </StyledButtons>
      </StyledContent>
    </Wrapper>
  );
